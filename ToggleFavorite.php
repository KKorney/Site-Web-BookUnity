...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['livre_id'])) {
    ...
    // Récupérer la valeur actuelle de est_favoris
    $stmt = $pdo->prepare("SELECT est_favoris FROM bibliotheque WHERE id_livre = :livre_id AND id_utilisateur = :user_id");
    $stmt->execute([':livre_id' => $livre_id, ':user_id' => $user_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($result) {
        $nouveau_status = $result['est_favoris'] ? 0 : 1; // toggle
        $stmt = $pdo->prepare("UPDATE bibliotheque SET est_favoris = :status WHERE id_livre = :livre_id AND id_utilisateur = :user_id");
